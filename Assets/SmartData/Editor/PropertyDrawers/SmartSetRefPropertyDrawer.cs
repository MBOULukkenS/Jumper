	...
	public class SmartSetRefPropertyDrawer : SmartRefPropertyDrawerBase {
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label){
			if (_useList && !_forceEventable){				
				SerializedProperty p = property.FindPropertyRelative(_listProp);
		...
		protected override void DrawGUI(Rect position, SerializedProperty property, GUIContent label, Vector2 min, Vector2 max){
			...
			if (_forceEventable){
				...
				SerializedProperty useListProp = property.FindPropertyRelative("_useList");
				...
				if (_useList){
					...
					SerializedProperty lProp = property.FindPropertyRelative(_listProp);
