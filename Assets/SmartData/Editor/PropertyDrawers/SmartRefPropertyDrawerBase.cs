...
namespace SmartData.Editors {
	public abstract class SmartRefPropertyDrawerBase : PropertyDrawer {
		...
		public sealed override void OnGUI(Rect position, SerializedProperty property, GUIContent label){
			...
			SerializedProperty ownerProp = property.FindPropertyRelative("_owner");
			Component owner = (Component)property.serializedObject.targetObject;
			if (ownerProp.objectReferenceValue != owner){
				ownerProp.objectReferenceValue = owner;
				metadataGenerated = true;
			}
			SerializedProperty pathProp = property.FindPropertyRelative("_propertyPath");
		...
		protected void DrawEvent(SerializedProperty property, Rect position, Vector2 min, Vector2 max, bool forceExpand){
			...
			if (e0 != null){
				...
				if (_showEvent){
					...
					SerializedProperty autoListen = property.FindPropertyRelative("_autoListen");
		...
		protected bool IsWriteable(SerializedProperty p, FieldInfo f){
			...
			MethodInfo w = pType.GetMethod("_EDITOR_GetIsWritable", BindingFlags.NonPublic | BindingFlags.Instance);
			object result = w.Invoke(p.GetObject(), null);
			return (bool)result;
		}
		protected bool IsForceEventable(SerializedProperty p, FieldInfo f, out bool forceExpand){
			...
			object[] ats = GetCustomAttributes(p, fieldInfo, typeof(ForceEventableAttribute), true);
		...
		protected bool IsForceHideEvent(SerializedProperty p, FieldInfo f){
			object[] ats = GetCustomAttributes(p, fieldInfo, typeof(ForceHideEventAttribute), true);
		...
		protected bool IsForceNoAutoListen(SerializedProperty p, FieldInfo f, out bool hide){
			...
			object[] ats = GetCustomAttributes(p, fieldInfo, typeof(ForceNoAutoListenAttribute), true);
			if (ats != null && ats.Length > 0){
		...
		protected bool IsForceNoMultiIndex(SerializedProperty p, FieldInfo f){
			...
			object[] ats = GetCustomAttributes(p, fieldInfo, typeof(ForceNoMultiIndexAttribute), true);
			return (ats != null && ats.Length > 0);
		}

		protected object[] GetCustomAttributes(SerializedProperty property, FieldInfo field, System.Type type, bool inherit){
			object[] ats = field.GetCustomAttributes(type, true);
