	...
	public abstract class SmartObjectEditorBase : Editor {
		...
		public sealed override void OnInspectorGUI(){
			...
			if (!_const){
				...
				int count = 0;
				foreach (KeyValuePair<GameObject, List<RefData>> a in _refs){
					count += a.Value.Count;
				}
		...
		protected bool IsOnRestoreOverridden(System.Type t){
			...
			foreach (MethodInfo m in t.GetMethods(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy)){
		...
		protected virtual void DrawListeners(){
			...
			--EditorGUI.indentLevel;
			if (_showListeners){
				foreach (KeyValuePair<object, List<MethodInfo>> a in _listeners){
			...
			EditorGUILayout.BeginVertical(EditorStyles.helpBox);{
				...
				if (_decorators.isExpanded){
					...
					for (int i=0; i<_decorators.arraySize; ++i){
						SerializedProperty pd = _decorators.GetArrayElementAtIndex(i);
						SmartDecoratorBase decorator = (SmartDecoratorBase)pd.objectReferenceValue;
						// Array elements are just references
						// Need a separate SO to edit the actual instance
						SerializedObject so = new SerializedObject(decorator);

						// Nice(r) type name
						Type dt = decorator.GetType();
						string dn = dt.Name;
						...
						EditorGUILayout.BeginHorizontal(); {
							SerializedProperty spa = so.FindProperty("_active");
							...
							if (GUILayout.Button(_iconChevronUp, _decBtnStyle)){
								changed = true;
								Object d0 = _decorators.GetArrayElementAtIndex(i-1).objectReferenceValue;
							...
							if (GUILayout.Button(_iconChevronDown, _decBtnStyle)){
								changed = true;
								Object d1 = _decorators.GetArrayElementAtIndex(i+1).objectReferenceValue;
								_decorators.GetArrayElementAtIndex(i+1).objectReferenceValue = decorator;
					...
					if (_showAddDecorators){
						...
						bool found = false;
						foreach (Type t in _decoratorTypes){
							// Filter
							if (string.IsNullOrEmpty(_decoratorFilter) || t.Name.ToLower().Contains(_decoratorFilter.ToLower())){
								...
								EditorGUILayout.BeginHorizontal(); {
									...
									if (GUILayout.Button(new GUIContent(t.Name, GetDecoratorDescription(t)))){
										changed = true;
										// Create decorator
										ScriptableObject s = ScriptableObject.CreateInstance(t);
		...
		string GetDecoratorDescription(System.Type tDecorator){
			foreach (object d in tDecorator.GetCustomAttributes(typeof(SmartData.DecoratorDescriptionAttribute), true)){
		...
		protected uint PopulateListeners(object smart){
			...
			object arr = _getListeners.GetValue(relay);
			uint c = (uint)_getListenerCount.GetValue(relay, null);
			Array lsnrs = (System.Array)arr;
			...
			for (int i=0; i<c; ++i){
				...
				if (v != null){
					...
					object tgt = delTarget.GetValue(v, null);
					MethodInfo mth = (MethodInfo)delMethod.GetValue(v, null);
		...
		void PopulateRefs(){
			_refsToRemove.Clear();
			_refs.Clear();
			foreach (KeyValuePair<WeakReference, Type> r in SmartDataRegistry.GetSmartReferences()){
				if (!r.Key.IsAlive){
			...
			// Sweep marked refs
			foreach (WeakReference r in _refsToRemove){
