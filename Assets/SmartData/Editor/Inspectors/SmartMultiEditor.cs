...
namespace SmartData.Editors {
	...
	public class SmartMultiEditor : SmartObjectEditorBase {
		...
		protected override void OnEnable(){
			...
			MethodInfo gdt = t.GetMethod("_EDITOR_GetDataType", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy);
		...
		protected override void DrawInspector(){
			...
			if (Application.isPlaying) {
				SmartMultiBase multi = (target as SmartMultiBase);
				int c = multi.count;

				EditorGUILayout.Space();
				EditorGUILayout.LabelField("Count: "+c.ToString());
				
				GUILayoutOption wIndex = GUILayout.Width(50);
				GUILayoutOption wValue = GUILayout.Width(50);
				...
				for (int i=0; i<c; ++i){
					...
					if (_getValue != null){
						_args[0] = i;
						object val = _getValue.Invoke(multi, _args);
						if (val != null){
