...
namespace SmartData.Graph
{
	...
	public static class SmartGraphPopulator
	{
		...
		public static List<SmartGraphConnection> FindAllEvents()
		{
			...
			#region SmartRefs
			_refsToRemove.Clear();
			Dictionary<WeakReference, Type> refs = SmartData.Editors.SmartDataRegistry.GetSmartReferences();
			foreach (KeyValuePair<WeakReference, Type> r in refs){
				if (!r.Key.IsAlive){
			...
			Type tm = typeof(SmartData.Abstract.SmartMultiBase); 
			foreach (KeyValuePair<SmartBase, Type> a in SmartData.Editors.SmartDataRegistry.GetSmartDatas()){
				if (tm.IsAssignableFrom(a.Value)){
					Array persistent = (System.Array)a.Key.GetType().GetFieldPrivate("_persistent", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(a.Key);
					if (persistent != null){
						int i=0;
						foreach (object p in persistent){
							Object rec = p as Object;
							calls.Add(
								new SmartGraphConnection (
									a.Key, 
									rec, 
		...
		private static Object TryGetSmartObject(object r, System.Type t, string fieldName, bool isProperty){
			if (isProperty)
			{
				PropertyInfo p = t.GetProperty(fieldName, BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
				if (p != null)
				{
					object result = p.GetValue(r, null);
					if (result != null){
			...
			{
				FieldInfo f = t.GetFieldPrivate(fieldName, BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
				if (f != null)
				{
					object result = f.GetValue(r);
					if (result != null){
						return result as Object;
					}
				}
