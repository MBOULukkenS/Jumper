		...
		public static object GetObject(this SerializedProperty p){
			...
			for (int i=0; i<elements.Length; ++i){
				if (p.IsArrayElement(elements[i])){
					...
					FieldInfo fi = o.GetType().GetFieldPrivate(arrayName, FLAGS_ALL);
					Array arr = (System.Array)fi.GetValue(o);
					o = arr.GetValue(j);
				} else {
					FieldInfo fi = o.GetType().GetFieldPrivate(elements[i], FLAGS_ALL);
					o = fi.GetValue(o);
				}
			}
			return o;
