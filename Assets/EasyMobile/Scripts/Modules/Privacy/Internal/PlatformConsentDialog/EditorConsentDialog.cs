...
namespace EasyMobile.Internal.Privacy
{
    internal class EditorConsentDialog : IPlatformConsentDialog
    {
        ...
        {
            foreach (ConsentDialogContentSerializer.SplitContent content in DeserializeContents(contents))
            {
                ...
                {
                    ConsentDialog.Button buttonData = JsonUtility.FromJson<ConsentDialog.Button>(content.content);
                    dialog.AddButton(buttonData);
                }
                else if (content.IsToggle())
                {
                    ConsentDialog.Toggle toggleData = JsonUtility.FromJson<ConsentDialog.Toggle>(content.content);
        ...
        private IEnumerable<ConsentDialogContentSerializer.SplitContent> DeserializeContents(string contents)
        {
            ...
            List<object> jsonArray = Json.Deserialize(contents) as List<object>;
            ...
            foreach (object element in jsonArray)
            {
                ...
                ConsentDialogContentSerializer.SplitContent serializedObject = JsonUtility.FromJson<ConsentDialogContentSerializer.SplitContent>(element.ToString());
