...
namespace EasyMobile.Internal.Privacy
{
    internal abstract class BaseNativeEEARegionValidator : IPlatformEEARegionValidator
    {
        ...
        public IEnumerator ValidateEEARegionStatusCoroutine(List<EEARegionValidationMethods> methods, Action<EEARegionStatus> callback)
        {
            ...
            Stack<EEARegionStatus> results = new Stack<EEARegionStatus>();

            foreach (EEARegionValidationMethods method in methods.Distinct()) // Remove duplicate elements.
            {
            ...
            EEARegionStatus result = results.Contains(EEARegionStatus.NotInEEA) ? EEARegionStatus.NotInEEA : EEARegionStatus.Unknown;
            if (callback != null)
                callback(result);
        ...
        protected virtual IEnumerator ValidateViaGoogleServiceCoroutine(Stack<EEARegionStatus> resultsStack)
        {
            ...
            else
            {
                try
                {
                    GoogleServiceResponse responce = JsonUtility.FromJson<GoogleServiceResponse>(www.downloadHandler.text);
                    if (responce == null)
                    {
