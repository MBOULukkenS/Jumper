...
namespace EasyMobile.Internal.NativeAPIs.Media
{
    internal class AndroidPickFromGalleryProxy : AndroidJavaProxy
    {
            ...
            RuntimeHelper.RunOnMainThread(() =>
                {
                    ...
                    if (results != null)
                    {
                        MediaResult[] mediaResults = new MediaResult[results.Length];
        ...
        {
            string error = javaArgs[0] != null ? javaArgs[0].Call<string>("toString") : null;
            AndroidJavaObject[] results = javaArgs[1] != null ? AndroidJNIHelper.ConvertFromJNIArray<AndroidJavaObject[]>(javaArgs[1].GetRawObject()) : null;
