...
namespace EasyMobile.Internal.Gif.Android
{
    internal class AndroidNativeGif
    {
        ...
        private static void GifExportCompletedCallback(int taskId, string filepath)
        {
            ...
            foreach (GCHandle gch in gcHandles[taskId])
            {
                gch.Free();
            }
        ...
        {
            AndroidNativeGif instance = new AndroidNativeGif(exportTask);
            Thread worker = new Thread(instance.DoExportGif);
        ...
        {
            int taskId = mExportTask.taskId;
            string filepath = mExportTask.filepath;
            int width = mExportTask.clip.Width;
            int height = mExportTask.clip.Height;
            int loop = mExportTask.loop;
            int fps = mExportTask.clip.FramePerSecond;
            int sampleFac = mExportTask.sampleFac;
            int frameCount = mExportTask.clip.Frames.Length;
            Color32[][] imageData = mExportTask.imageData;

            GCHandle[] gcHandleArray = new GCHandle[imageData.Length];
            IntPtr[] ptrArray = new IntPtr[imageData.Length];
        ...
        private static void GetFrameMetadataHolderFunc(int taskId, int frameCount, IntPtr pointerHolder)
        {
            ...
            GCHandle[] gcHandleArray = new GCHandle[frameCount];
            IntPtr[] ptrArray = new IntPtr[frameCount];
        ...
        private static void GetImageDataHolderFunc(int taskId, int frameCount, int frameWidth, int frameHeight, IntPtr pointerHolder)
        {
            ...
            Color32[][] buff = new Color32[frameCount][];
        ...
        private static void GifDecodingCompleteCallback(int taskId)
        {
            ...
            GifDecodeResources taskResources = DecodeTasks[taskId];
            ...
            try
            {
                ...
                {
                    DecodeCompleteCallback callback = taskResources.completeCallback;  // cache the callback reference as we'll reset taskResources soon.
            ...
            finally
            {
                ...
                if (taskResources.frameMetadataHandles != null)
                    foreach (GCHandle handle in taskResources.frameMetadataHandles)
                        handle.Free();

                if (taskResources.imageDataHandles != null)
                    foreach (GCHandle handle in taskResources.imageDataHandles)
                        handle.Free();
        ...
        {
            GifDecodeTask decodeTask = new GifDecodeTask()
            {
                taskId = taskId,
                filepath = filepath,
            ...
            AndroidNativeGif instance = new AndroidNativeGif(decodeTask);
            Thread worker = new Thread(instance.DoDecodeGif);
        ...
        {
            int taskId = mDecodeTask.taskId;
            string filepath = mDecodeTask.filepath;
            int framesToRead = mDecodeTask.framesToRead;
            DecodeCompleteCallback completeCallback = mDecodeTask.completeCallback;

            // First pin the gif metadata holding struct so we can pass it to unmanaged.
            GCHandle gifMetaHolderHandle = GCHandle.Alloc(new GifMetadata(), GCHandleType.Pinned);
