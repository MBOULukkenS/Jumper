...
namespace EasyMobile
{
    ...
    public class Advertising : MonoBehaviour
    {
        ...
        private static void LoadAllInterstitialAds(List<IAdClient> clients)
        {
            ...
            foreach (IAdClient client in clients)
            {             
                /// Load all defined interstitial ad placements.
                List<AdPlacement> customPlacements = client.DefinedCustomInterstitialAdPlacements;
                ...
                foreach (AdPlacement placement in customPlacements)
                {
        ...
        private static void LoadAllRewardedAds(List<IAdClient> clients)
        {
            foreach (IAdClient client in clients)
            {
                ...
                List<AdPlacement> customPlacements = client.DefinedCustomRewardedAdPlacements;
        ...
        private static List<IAdClient> GetAvailableNetworks<T>()
        {
            ...
            foreach (T network in Enum.GetValues(typeof(T)))
            {
                ...
                {
                    AdClientImpl workableClient = GetWorkableAdClient((AdNetwork)(Enum.Parse(typeof(T), network.ToString())));
        ...
        private static void DestroyAllBannerAds()
        {
            foreach (KeyValuePair<AdNetwork, List<AdPlacement>> pair in activeBannerAds)
            {
                ...
                {
                    AdClientImpl client = GetWorkableAdClient(pair.Key);
                    foreach (AdPlacement placement in pair.Value)
                        client.DestroyBannerAd(placement);
                }
            }
