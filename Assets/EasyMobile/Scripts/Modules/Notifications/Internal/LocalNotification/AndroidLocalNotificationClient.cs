...
namespace EasyMobile.Internal.Notifications
{
    internal class AndroidLocalNotificationClient : ILocalNotificationClient
    {
        ...
        public void Init(NotificationsSettings settings, INotificationListener listener)
        {
            ...
            string categoryGroupJson = AndroidNotificationHelper.ToJson(settings.CategoryGroups);
            ...
            string categoriesJson = AndroidNotificationHelper.ToJson(categories.ToArray());

            // Listener info
            string name = listener.Name;
            string backgroundNotificationMethod = ReflectionUtil.GetMethodName(listener.NativeNotificationFromBackgroundHandler);
            string foregroundNotificationMethod = ReflectionUtil.GetMethodName(listener.NativeNotificationFromForegroundHandler);
        ...
        public void GetPendingLocalNotifications(Action<NotificationRequest[]> callback)
        {
            ...
            AndroidNotificationNative._GetPendingLocalNotifications(
                androidRequests =>
                {
                    NotificationRequest[] requests = new NotificationRequest[androidRequests.Length];
