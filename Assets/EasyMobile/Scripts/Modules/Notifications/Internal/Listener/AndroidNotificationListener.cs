...
namespace EasyMobile.Internal.Notifications
{
    ...
    internal class AndroidNotificationListener : MonoBehaviour, INotificationListener
    {
        ...
        internal static AndroidNotificationListener GetListener()
        {
            if (sInstance == null)
            {
                GameObject go = new GameObject(ANDROID_NOTIFICATION_LISTENER_GAMEOBJECT);
        ...
        {
            AndroidNotificationResponse response = AndroidNotificationResponse.FromJson(jsonResponse);
            ...
            RuntimeHelper.RunOnMainThread(() =>
                {
                    ...
                    NotificationRequest request = response.request.ToCrossPlatformNotificationRequest();

                    LocalNotification delivered = new LocalNotification(
                                        request.id,
                                        actionId,
                                        request.content,
                                        isForeground,
