    ...
    internal partial class EM_SettingsEditor
    {
            ...
            DrawUppercaseSection("CATEGORIES_FOLDOUT_KEY", "CATEGORIES", () =>
                {
                    ...
                    {
                        foreach (NotificationCategory category in EM_Settings.Notifications.UserCategories)
                        {
        ...
        void AddNewNotificationCategoryGroup(SerializedProperty arrayProp)
        {
            if (arrayProp.isArray)
            {
                ...
                SerializedProperty newItem = arrayProp.GetArrayElementAtIndex(arrayProp.arraySize - 1);
        ...
        {
            SerializedProperty id = prop.FindPropertyRelative(NotificationCategory_Id);
            SerializedProperty groupId = prop.FindPropertyRelative(NotificationCategory_GroupId);
            SerializedProperty name = prop.FindPropertyRelative(NotificationCategory_Name);
            SerializedProperty description = prop.FindPropertyRelative(NotificationCategory_Description);
            SerializedProperty importance = prop.FindPropertyRelative(NotificationCategory_Importance);
            SerializedProperty enableBadge = prop.FindPropertyRelative(NotificationCategory_EnableBadge);
            SerializedProperty lights = prop.FindPropertyRelative(NotificationCategory_Lights);
            SerializedProperty lightColor = prop.FindPropertyRelative(NotificationCategory_LightColor);
            SerializedProperty vibration = prop.FindPropertyRelative(NotificationCategory_Vibration);
            SerializedProperty vibrationPattern = prop.FindPropertyRelative(NotificationCategory_VibrationPattern);
            SerializedProperty lockScreenVisibility = prop.FindPropertyRelative(NotificationCategory_LockScreenVisibility);
            SerializedProperty sound = prop.FindPropertyRelative(NotificationCategory_Sound);
            SerializedProperty soundName = prop.FindPropertyRelative(NotificationCategory_SoundName);
            SerializedProperty actionButtons = prop.FindPropertyRelative(NotificationCategory_ActionButtons);
        ...
        {
            List<string> retList = new List<string>{ EM_Constants.NoneSymbol };
            SerializedProperty property = NotificationProperties.categoryGroups.property;
            ...
            if (property != null && property.isArray)
            {
                ...
                {
                    SerializedProperty group = property.GetArrayElementAtIndex(i);
        ...
        string[] BuildListOfNotificationCategoryGroupsName(IDictionary<string, string> categoryGroupsDict)
        {
            ...
            string[] list = new string[categoryGroupsDict.Count + 1];
        ...
        void ImportAndroidNotificationResFolder()
        {
            ...
            if (!string.IsNullOrEmpty(selectedFolder))
            {
                        ...
                        "Cancel"))
                {
                    ...
                    EM_AndroidLibBuilder.AndroidLibConfig config = new EM_AndroidLibBuilder.AndroidLibConfig();
