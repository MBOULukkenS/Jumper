...
namespace EasyMobile.Editor
{
    ...
    public class AdPlacementDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            SerializedProperty useCustomNameProp = property.FindPropertyRelative(UseCustomNameField);

            float labelWidth = EditorGUIUtility.labelWidth;
            float lineHeight = EditorGUIUtility.singleLineHeight;
            float toggleWidth = position.width * 0.3f;
            int toggleOffset = 15;  // for the toggle to be drawn nearer the placement rect    

            Rect placementRect = new Rect(position.x, position.y, position.width - toggleWidth, lineHeight);
            Rect toggleRect = new Rect(placementRect.x + placementRect.width - toggleOffset, placementRect.y, toggleWidth + toggleOffset, lineHeight);   
            ...
            else    // if the user wants to choose among one of the built-in placements.
            {
                ...
                int lastIndex = Array.IndexOf(BuiltinPlacementNames, nameProp.stringValue);

                // Now draw the popup and store the result.
                int indexResult = EditorGUI.Popup(
                                      placementRect,
                                      label, 
