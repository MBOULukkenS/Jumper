...
namespace EasyMobile.Editor
{
    ...
    public class EM_PluginManager : AssetPostprocessor
    {
        ...
        internal static void CheckModules()
        {
            ...
            {
                ModuleManager manager = GetModuleManager(mod);

                if (manager == null)
                    continue;

                // Is this composite module?
                CompositeModuleManager compManager = manager as CompositeModuleManager;
        ...
        {
            List<AndroidPermission> modulePermissions = new List<AndroidPermission>();
            ModuleManager manager = GetModuleManager(mod);
        ...
        {
            Dictionary<Module, List<AndroidPermission>> allPermissions = new Dictionary<Module, List<AndroidPermission>>();

            foreach (Module mod in Enum.GetValues(typeof(Module)))
            {
                List<AndroidPermission> modulePermissions = GetAndroidPermissionsRequiredByModule(mod);
        ...
        {
            List<iOSInfoPlistItem> moduleInfoItems = new List<iOSInfoPlistItem>();
            ModuleManager manager = GetModuleManager(mod);
        ...
        {
            Dictionary<Module, List<iOSInfoPlistItem>> allInfoItems = new Dictionary<Module, List<iOSInfoPlistItem>>();

            foreach (Module mod in Enum.GetValues(typeof(Module)))
            {
                List<iOSInfoPlistItem> moduleInfoItems = GetIOSInfoItemsRequiredByModule(mod);
