...
namespace UnityEngine.UI.Extensions
{
	 ...
	 */
		void SetSelectableGroup(IEnumerable<MonoBehaviour> behaviourCollection) {
			...
			foreach(MonoBehaviour behaviour in behaviourCollection) {
		...
		void CreateBoxRect(){
			GameObject selectionBoxGO = new GameObject();
		...
		IBoxSelectable GetSelectableAtMousePosition() {
			...
			foreach (IBoxSelectable selectable in selectables) {
				...
				if (rectTransform) {
					...
					Camera screenCamera = GetScreenPointCamera(rectTransform);
					...
					float radius = selectable.transform.GetComponent<UnityEngine.Renderer>().bounds.extents.magnitude;
					
					Vector2 selectableScreenPoint = GetScreenPointOfSelectable(selectable);
	 ...
	 */
		...
		public IBoxSelectable[] GetAllSelected(){
			...
			List<IBoxSelectable> selectedList = new List<IBoxSelectable>();
