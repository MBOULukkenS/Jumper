...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UILineRenderer : UIPrimitiveBase
	{
		...
		private void PopulateMesh(VertexHelper vh, Vector2[] pointsToDraw)
		{
			...
			float sizeX = !relativeSize ? 1 : rectTransform.rect.width;
			float sizeY = !relativeSize ? 1 : rectTransform.rect.height;
			...
			for (int i = 0; i < segments.Count; i++) {
				if (!lineList && i < segments.Count - 1) {
					...
					float miterDistance = lineThickness / (2 * Mathf.Tan (angle / 2));
					Vector3 miterPointA = segments [i] [2].position - vec1.normalized * miterDistance * sign;
					Vector3 miterPointB = segments [i] [3].position + vec1.normalized * miterDistance * sign;
