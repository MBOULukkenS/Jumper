...
namespace UnityEngine.UI.Extensions
{
    public abstract class FancyScrollView<TData, TContext> : MonoBehaviour where TContext : class
    {
        ...
        protected void UpdatePosition(float position, bool forceUpdateContents = false)
        {
            ...
            float visibleMinPosition = position - (cellOffset / cellInterval);
            float firstCellPosition = (Mathf.Ceil(visibleMinPosition) - visibleMinPosition) * cellInterval;
            int dataStartIndex = Mathf.CeilToInt(visibleMinPosition);
            int count = 0;
            ...
            for (float p = firstCellPosition; p <= 1f; p += cellInterval, count++)
            {
                int dataIndex = dataStartIndex + count;
                FancyScrollViewCell<TData, TContext> cell = cells[GetCircularIndex(dataIndex, cells.Count)];
        ...
        {
            GameObject cellObject = Instantiate(cellBase, cellContainer);
            FancyScrollViewCell<TData, TContext> cell = cellObject.GetComponent<FancyScrollViewCell<TData, TContext>>();
