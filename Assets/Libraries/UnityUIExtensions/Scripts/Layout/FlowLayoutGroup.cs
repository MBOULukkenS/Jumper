	...
	public class FlowLayoutGroup : LayoutGroup
	{
		...
		public override void CalculateLayoutInputVertical()
		{
			if (startAxis == Axis.Horizontal) {
				...
				base.CalculateLayoutInputHorizontal ();
				float minHeight = GetGreatestMinimumChildHeigth () + padding.bottom + padding.top;
		...
		protected void LayoutRow(IList<RectTransform> contents, float rowWidth, float rowHeight, float maxWidth, float xOffset, float yOffset, int axis)
		{
			...
			for (int j = 0; j < _itemList.Count; j++) {
				...
				int index = IsLowerAlign ? _itemList.Count - 1 - j : j;

				RectTransform rowChild = _itemList[index];

				float rowChildWidth = LayoutUtility.GetPreferredSize(rowChild, 0) + extraWidth;
		...
		protected void LayoutCol(IList<RectTransform> contents, float colWidth, float colHeight, float maxHeight, float xOffset, float yOffset, int axis)
		{
			...
			for (int j = 0; j < _itemList.Count; j++) {
				...
				int index = IsRightAlign ? _itemList.Count - 1 - j : j;

				RectTransform rowChild = _itemList[index];

				float rowChildWidth = LayoutUtility.GetPreferredSize(rowChild, 0) ;
				float rowChildHeight = LayoutUtility.GetPreferredSize(rowChild, 1) + extraHeight;
