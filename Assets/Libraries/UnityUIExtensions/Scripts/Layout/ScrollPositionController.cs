...
namespace UnityEngine.UI.Extensions
{
    public class ScrollPositionController : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void IDragHandler.OnDrag(PointerEventData eventData)
        {
            ...
            float position = (directionOfRecognize == ScrollDirection.Horizontal ? -pointerDelta.x : pointerDelta.y)
                           / GetViewportSize()
                           * scrollSensitivity
        ...
        void Update()
        {
            ...
            float offset = CalculateOffset(currentScrollPosition);
            ...
            if (autoScrollState.Enable)
            {
                ...
                {
                    float alpha = Mathf.Clamp01((Time.unscaledTime - autoScrollState.StartTime) / Mathf.Max(autoScrollState.Duration, float.Epsilon));
            ...
            {
                float position = currentScrollPosition;
            ...
            {
                float newVelocity = (currentScrollPosition - prevScrollPosition) / deltaTime;
        ...
        {
            float diff = GetCircularPosition(index, dataCount)
