...
namespace UnityEditor.UI
{
    ...
    static internal class ExtensionMenuOptions
	{
		...
		static public void AddUIVerticallScroller(MenuCommand menuCommand)
		{
			...
			UIVerticalScroller uiscr = uiVerticalScrollerRoot.AddComponent<UIVerticalScroller>();
			...
			for (int i = 0; i < 10; i++)
			{
				...
				CanvasGroup childCG = childPage.AddComponent<CanvasGroup>();
		...
		static public void AddAutoCompleteComboBox(MenuCommand menuCommand)
		{
			...
			//Setup ComboBox
			AutoCompleteComboBox autoCompleteComboBox = autoCompleteComboBoxRoot.AddComponent<AutoCompleteComboBox>();
			RectTransform cbbRT = autoCompleteComboBoxRoot.GetComponent<RectTransform>();

			//Setup Template
			itemTemplate.name = "ItemTemplate";
			RectTransform itemTemplateRT = itemTemplate.GetComponent<RectTransform>();
			itemTemplateRT.sizeDelta = cbbRT.sizeDelta;
			Button itemTemplateButton = itemTemplate.GetComponent<Button>();
			itemTemplateButton.transition = Selectable.Transition.None;
			LayoutElement itemTemplateLayoutElement = itemTemplate.AddComponent<LayoutElement>();
			...
			RectTransform overlayScrollPanelRT = overlayScrollPanel.GetComponent<RectTransform>();
			...
			ScrollRect scrollRect = overlayScrollPanel.AddComponent<ScrollRect>();
			scrollRect.horizontal = false;
			scrollRect.movementType = ScrollRect.MovementType.Clamped;
			scrollRect.verticalScrollbar = overlayScrollPanelScrollBar.GetComponent<Scrollbar>();
			//Overlay Items list
			RectTransform overlayScrollPanelItemsRT = overlayScrollPanelItems.GetComponent<RectTransform>();
			...
			VerticalLayoutGroup overlayScrollPanelItemsVLG = overlayScrollPanelItems.AddComponent<VerticalLayoutGroup>();
			overlayScrollPanelItemsVLG.padding = new RectOffset(0, 0, 5, 0);
			ContentSizeFitter overlayScrollPanelItemsFitter = overlayScrollPanelItems.AddComponent<ContentSizeFitter>();
			overlayScrollPanelItemsFitter.verticalFit = ContentSizeFitter.FitMode.MinSize;
			//Overlay Scrollbar
			RectTransform overlayScrollPanelScrollbarRT = overlayScrollPanelScrollBar.GetComponent<RectTransform>();
			...
			RectTransform arrowButtonRT = arrowButton.GetComponent<RectTransform>();
		...
		static public void AddDropDownList(MenuCommand menuCommand)
		{
			...
			//Setup DropDownList
			DropDownList dropDownList = dropDownListRoot.AddComponent<DropDownList>();
			RectTransform cbbRT = dropDownListRoot.GetComponent<RectTransform>();
			...
			itemTemplate.SetActive(false);
			//Item Template Image
			RectTransform itemTemplateImageRT = itemTemplateImage.GetComponent<RectTransform>();
			...
			RectTransform mainButtonRT = mainButton.GetComponent<RectTransform>();
			...
			Text mainButtonText = mainButton.GetComponentInChildren<Text>();
			mainButtonText.alignment = TextAnchor.MiddleLeft;
			mainButtonText.text = "Select Item...";
			RectTransform mainButtonTextRT = mainButtonText.GetComponent<RectTransform>();
			...
			//Main Button Image
			RectTransform mainButtonImageRT = mainButtonImage.GetComponent<RectTransform>();
			...
			RectTransform arrowTextRT = arrowText.GetComponent<RectTransform>();
			...
			Text arrowTextComponent = arrowText.GetComponent<Text>();
			arrowTextComponent.text = "▼";
			arrowTextComponent.alignment = TextAnchor.MiddleCenter;
			CanvasGroup arrowTextCanvasGroup = arrowText.AddComponent<CanvasGroup>();
		...
		static public void AddBoundTooltip(MenuCommand menuCommand)
		{
			...
			BoundTooltipTrigger tooltip = go.AddComponent<BoundTooltipTrigger>();
			tooltip.text = "This is my Tooltip Text";
			Image boundTooltip = go.AddComponent<Image>();
		...
		{
			BoundTooltipItem btti = Object.FindObjectOfType<BoundTooltipItem>();
			if (btti == null)
			{
				GameObject boundTooltipItem = CreateUIObject("ToolTipItem", parent.GetComponentInParent<Canvas>().gameObject);
				btti = boundTooltipItem.AddComponent<BoundTooltipItem>();
				CanvasGroup boundTooltipItemCanvasGroup = boundTooltipItem.AddComponent<CanvasGroup>();
				boundTooltipItemCanvasGroup.interactable = false;
				boundTooltipItemCanvasGroup.blocksRaycasts = false;
				Image boundTooltipItemImage = boundTooltipItem.AddComponent<Image>();
				boundTooltipItemImage.sprite = AssetDatabase.GetBuiltinExtraResource<Sprite>(kBackgroundSpriteResourcePath);
				GameObject boundTooltipItemText = CreateUIObject("Text", boundTooltipItem);
				RectTransform boundTooltipItemTextRT = boundTooltipItemText.GetComponent<RectTransform>();
				...
				Text boundTooltipItemTextcomponent = boundTooltipItemText.AddComponent<Text>();
        ...
        static public void AddSegmentedControl(MenuCommand menuCommand)
        {
            ...
            string[] labels = new string[] { "This", "That", "Other" };
            for (int i = 0; i < 3; i++)
			{
                Button button = AddButtonAsChild(go).GetComponent<Button>();
        ...
        static public void AddStepper(MenuCommand menuCommand)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                GameObject button = AddButtonAsChild(go);
