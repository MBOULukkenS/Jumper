...
namespace Doozy.Editor.Drawers
{
    ...
    public class RangedFloatDrawer : BaseDrawer
    {
        ...
        private void Draw(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            GUIContent minValueContent = new GUIContent("min " + minValue.ToString("F2"));
            GUIContent maxValueContent = new GUIContent(maxValue.ToString("F2") + " max");
            ...
            Rect minValueLabelRect = new Rect(position.xMin, position.y + (DGUI.Properties.SingleLineHeight - minValueContentSize.y) / 2, minValueContentSize.x, minValueContentSize.y);
            Rect maxValueLabelRect = new Rect(position.xMax - maxValueContentSize.x, position.y + (DGUI.Properties.SingleLineHeight - maxValueContentSize.y) / 2, maxValueContentSize.x, maxValueContentSize.y);
            Rect sliderRect = new Rect(position.xMin + minValueContentSize.x + DGUI.Properties.Space(2),
                                      position.y - 1,
