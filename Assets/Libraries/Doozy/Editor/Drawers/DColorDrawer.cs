    ...
    public class DColorDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            EditorGUI.BeginProperty(position, label, property);
            {
                ...
                //calculate rects
                Rect rect = new Rect(position.x, position.y, position.width, DGUI.Properties.SingleLineHeight);
                Rect colorNameRect = new Rect(rect.x, rect.y, NAME_LABEL_WIDTH, rect.height);
                float colorFieldWidth = (rect.width - NAME_LABEL_WIDTH - DGUI.Properties.StandardVerticalSpacing * 3) / 3;
                Rect lightColorRect = new Rect(colorNameRect.x + NAME_LABEL_WIDTH + DGUI.Properties.StandardVerticalSpacing, rect.y, colorFieldWidth, rect.height);
                Rect normalColorRect = new Rect(lightColorRect.x + colorFieldWidth + DGUI.Properties.StandardVerticalSpacing, rect.y, colorFieldWidth, rect.height);
                Rect darkColorRect = new Rect(normalColorRect.x + colorFieldWidth + DGUI.Properties.StandardVerticalSpacing, rect.y, colorFieldWidth, rect.height);
