...
namespace Doozy.Editor.Nody.NodeGUI
{
    ...
    public class SwitchBackNodeGUI : BaseNodeGUI
    {
        ...
        protected override void OnNodeGUI()
        {
            ...
            //Draw Target
            Rect targetIconRect = new Rect(x + width / 2 - TITLE_ICON_SIZE / 2, DynamicHeight, TITLE_ICON_SIZE, TITLE_ICON_SIZE);
            DynamicHeight += targetIconRect.height;
            DynamicHeight += DGUI.Properties.Space();
            Vector2 targetContentSize = TargetContentSize;
            Rect targetTextRect = new Rect(x + width / 2 - targetContentSize.x / 2, DynamicHeight, targetContentSize.x, targetContentSize.y);
            ...
            //Draw Sources
            Rect sourcesIconRect = new Rect(x + width / 2 - TITLE_ICON_SIZE / 2, DynamicHeight, TITLE_ICON_SIZE, TITLE_ICON_SIZE);
            DynamicHeight += sourcesIconRect.height;
            DynamicHeight += DGUI.Properties.Space();
            Vector2 sourcesContentSize = SourcesContentSize;
            Rect sourcesTextRect = new Rect(x + width / 2 - sourcesContentSize.x / 2, DynamicHeight, sourcesContentSize.x, sourcesContentSize.y);
            DynamicHeight += targetTextRect.height;
            DynamicHeight += TitleVerticalSpacing;
            {
                GUI.color = GUI.color.WithAlpha(TITLE_ALPHA * color.a);
            ...
            {
                GUIContent sourceNameContent = new GUIContent(source.SourceName);
                Vector2 sourceNameContentSize = SourceNameStyle.CalcSize(sourceNameContent);
                Rect sourceNameRect = new Rect(x + width / 2 - sourceNameContentSize.x / 2, DynamicHeight + (DGUI.Properties.SingleLineHeight - sourcesContentSize.y) / 2, sourceNameContentSize.x, sourceNameContentSize.y);
                ...
                if (!string.IsNullOrEmpty(SwitchBackNode.ReturnSourceOutputSocketId) && SwitchBackNode.ReturnSourceOutputSocketId == source.OutputSocketId)
                {
                    ...
                    Rect leftChevronRect = new Rect(sourceNameRect.xMin - chevronWidth - DGUI.Properties.Space(2), sourceNameRect.y, chevronWidth, chevronHeight);
                    Rect rightChevronRect = new Rect(sourceNameRect.xMax + DGUI.Properties.Space(2), sourceNameRect.y, chevronWidth, chevronHeight);
