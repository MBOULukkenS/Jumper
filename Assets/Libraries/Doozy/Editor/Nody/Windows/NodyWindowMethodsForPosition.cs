...
namespace Doozy.Editor.Nody.Windows
{
    public partial class NodyWindow
    {
        ...
        private VirtualPoint GetVirtualPointAtWorldPosition(Vector2 worldPosition)
        {
            ...
            float size = 8;
            Rect mouseRect = new Rect(worldPosition.x,
                                     worldPosition.y,
                                     size,
                                     size);
...
//            GUI.color = Color.white;
            ...
            foreach (VirtualPoint point in PointsDatabase[key])
            {
                ...
                Rect pointGridRect = new Rect(point.Rect.position * CurrentZoom, point.Rect.size * CurrentZoom);
...
//                GUI.color = Color.white;
        ...
        private Socket GetSocketAtWorldPosition(Vector2 worldPosition)
        {
            ...
            foreach (Socket socket in SocketsDatabase.Values)
            {
                ...
                Rect socketWorldRect = new Rect(nodeParent.GetX(),
                                               nodeParent.GetY() + socket.GetY(),
                                               socket.GetWidth(),
                                               socket.GetHeight());

                Rect socketGridRect = new Rect(socketWorldRect.position * CurrentZoom + CurrentPanOffset,
        ...
        private Socket GetSocketAtWorldPositionFromHoverRect(Vector2 worldPosition)
        {
            ...
            foreach (Socket socket in SocketsDatabase.Values)
            {
                ...
                Rect socketWorldRect = new Rect(nodeParentGUI.Node.GetX() + (nodeParentGUI.Node.GetWidth() - socket.HoverRect.width) / 2, //(nodeParent.GetWidth() - socket.HoverRect.width) / 2 -> is the X offset of the hover rect
...
//                GUI.color = Color.white;
        ...
        private IEnumerable<Vector2> GetSocketConnectionPointsInWorldSpace(Socket socket, Node parentNode)
        {
            ...
            {
                Rect socketWorldRect = new Rect(parentNode.GetX(),
