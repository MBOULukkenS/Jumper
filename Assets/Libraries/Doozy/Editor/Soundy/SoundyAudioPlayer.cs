...
namespace Doozy.Editor.Soundy
{
    public static class SoundyAudioPlayer
    {
        ...
        public static Player GetPlayer(SoundGroupData soundGroupData, AudioMixerGroup audioMixerGroup = null)
        {
            ...
            Player player = new Player(soundGroupData, audioMixerGroup);
        ...
        public static Player GetPlayer(AudioClip audioClip, AudioMixerGroup audioMixerGroup = null)
        {
            ...
            Player player = new Player(audioClip, audioMixerGroup);
        ...
        public class Player
        {
            ...
            public void DrawPlayer(Rect rect, ColorName colorName)
            {
                ...
                Rect progressMouseDetectionArea = new Rect(rect.x + DGUI.Properties.Space(),
                                                          rect.y,
                ...
                GUIContent audioNameContent = new GUIContent(audioName);
                Vector2 audioNameContentSize = DGUI.Label.Style(Size.S).CalcSize(audioNameContent);

                Rect audioNameLabelRect = new Rect(rect.x + DGUI.Properties.Space(),
                                                  rect.y + (rect.height - audioNameContentSize.y) / 2 - DGUI.Properties.Space(),
                                                  audioNameContentSize.x,
                                                  audioNameContentSize.y);

                GUIContent durationContent = new GUIContent(DurationLabel);
                Vector2 durationContentSize = DGUI.Label.Style(Size.S).CalcSize(durationContent);

                Rect durationLabelRect = new Rect(rect.x + rect.width - durationContentSize.x - DGUI.Button.IconButton.Width * 2 - DGUI.Properties.Space(6),
...
//                float barHeight = mouseInsideScrubber ? DGUI.Properties.Space(2) : DGUI.Properties.Space();
                ...
                Rect barBackgroundRect = new Rect(rect.x + DGUI.Properties.Space(),
                                                 durationLabelRect.y + durationLabelRect.height + DGUI.Properties.Space(),
                                                 rect.width - DGUI.Properties.Space(7) - DGUI.Button.IconButton.Width * 2,
                                                 barHeight);

                Rect barProgressRect = new Rect(barBackgroundRect.x, barBackgroundRect.y, ProgressBarWidth(barBackgroundRect.width), barBackgroundRect.height);


                Rect stopButtonRect = new Rect(rect.x + rect.width - DGUI.Button.IconButton.Width - DGUI.Properties.Space(),
                                              rect.y + (rect.height - DGUI.Button.IconButton.Height) / 2,
                                              DGUI.Button.IconButton.Width,
                                              DGUI.Button.IconButton.Height);

                Rect playButtonRect = new Rect(stopButtonRect.x - DGUI.Button.IconButton.Width - DGUI.Properties.Space(),
                                              rect.y + (rect.height - DGUI.Button.IconButton.Height) / 2,
                                              DGUI.Button.IconButton.Width,
                                              DGUI.Button.IconButton.Height);
...
//                GUI.color = initialColor;
                ...
                {
                    GUIContent scrubberTimeContent = new GUIContent(GetTimePretty(AudioSource.clip.length * progressMousePosition)); //get the scrubber position time value in a pretty format 12:34
                    Vector2 scrubberTimeContentSize = DGUI.Label.Style(Size.S).CalcSize(scrubberTimeContent); //calculate the time label size
                    Rect scrubberTimeLabelRect = new Rect(mousePosition.x - scrubberTimeContentSize.x / 2 + DGUI.Properties.Space(), //calculate the time label rect
                                                         barBackgroundRect.y - scrubberTimeContentSize.y - DGUI.Properties.Space(2),
                                                         scrubberTimeContentSize.x,
                                                         scrubberTimeContentSize.y);
                    Rect scrubberTimeBackgroundRect = new Rect(scrubberTimeLabelRect.x - DGUI.Properties.Space(), //calculate a background for the time label to make it easy to read when hovering over other text 
