...
namespace Doozy.Editor.Internal
{
    public class BaseDrawer : PropertyDrawer
    {
        ...
        {
            Rect verticalHeaderRect = new Rect(drawRect.x, drawRect.y, DGUI.Properties.SingleLineHeight + DGUI.Properties.Space(2), PropertyHeight(NumberOfLines[property.propertyPath]));
            DGUI.Background.Draw(verticalHeaderRect, drawerColor);

            Rect iconRect = new Rect(verticalHeaderRect.x + DGUI.Properties.Space() * 1.5f,
        ...
        {
            //calculate rects
            Rect rect = new Rect(position.x, position.y, position.width, PropertyHeight(numberOfLines));
            Rect drawRect = new Rect(position.x, position.y, position.width, DGUI.Properties.SingleLineHeight);
