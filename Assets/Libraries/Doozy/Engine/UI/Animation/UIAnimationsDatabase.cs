...
#if UNITY_EDITOR
            UIAnimationDatabase asset = AssetDatabase.LoadAssetAtPath<UIAnimationDatabase>(Path.Combine(Path.Combine(DoozyPath.UIANIMATIONS_RESOURCES_PATH, DatabaseType.ToString()), UIAnimations.DEFAULT_DATABASE_NAME + ".asset"));
            if (asset != null)
            {
                Databases.Add(asset);
                UpdateDatabaseNames();
